...
class Install {
	...
	private static function maybeUpgradeOptions( $version = 'free' ) {
		...
		if ( ! $upgraded ) {
			if ( ( $version === 'free' && $freeSettingsVersion < 2 && $proSettingsVersion !== 2 )
	...
	public static function checkVersion() {
		if ( ! defined( 'IFRAME_REQUEST' ) ) {
			...
			if ( dgoraAsfwFs()->is__premium_only() ) {
				...
				if ( get_option( 'dgwt_wcas_version_pro' ) != DGWT_WCAS_VERSION ) {
					...
					// TODO Remove this in v1.22
					delete_option( Builder::LAST_BUILD_OPTION_KEY );
					delete_option( Builder::LAST_BUILD_OPTION_KEY . '_tmp' );
