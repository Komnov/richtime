...
class Database {
	...
	private static function install( $indexRoleSuffix = '' ) {
		...
		if ( file_exists( $upFile ) ) {
			...
			WPDB::query( "CREATE INDEX main_variation_id ON $tableName(variation_id);" );
			WPDB::query( "CREATE INDEX main_product_id ON $tableName(product_id);" );
			WPDB::query( "CREATE INDEX main_sku ON $tableName(sku);" );
			WPDB::query( "CREATE INDEX main_lang ON $tableName(lang);" );
		}
	}
	...
	public static function remove( $indexRoleSuffix = '' ) {
		...
		wp_cache_delete( 'table_exists', 'fibosearch_table_exists' );
	}
}
