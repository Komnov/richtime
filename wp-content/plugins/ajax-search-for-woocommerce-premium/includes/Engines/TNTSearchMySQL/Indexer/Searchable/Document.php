...
class Document extends AbstractDocument {
	...
	public function delete() {
		...
		WPDB::query( WPDB::prepare( "DELETE FROM $doclistTable WHERE doc_id = %d", $this->getID() ) );
	...
	private function saveWordlist() {
		...
		foreach ( $allWords as $key => $word ) {
			$term = WPDBSecond::get_row( WPDBSecond::prepare( "
                    SELECT id, num_hits
                    FROM $table
                    WHERE term = %s",
				$key ), ARRAY_A );
			...
			if ( empty( $term ) ) {
				...
				if ( ! empty( WPDBSecond::$db->insert_id ) ) {
					$termIds[] = WPDBSecond::$db->insert_id;
				}
			} else {
				$termIds[] = (int) $term['id'];
	...
	private function saveDoclist( $termdIds ) {
		...
		foreach ( $termdIds as $termId ) {
			...
			WPDBSecond::insert( $table, $data, $format );
		}
	}
}
