		...
		if ( file_exists( $upFile ) ) {
			...
			foreach ( $postTypes as $postType ) {
				foreach ( $langs as $lang ) {
					...
					WPDB::query( "CREATE INDEX main_term_id_index ON $doclistTableName(term_id);" );
					WPDB::query( "CREATE INDEX main_doc_id_index ON $doclistTableName(doc_id);" );

					if ( Helpers::doesDbSupportJson__premium_only() ) {
						$cacheTableName = Utils::getTableName( 'searchable_cache', $lang, $postType ) . Config::getIndexRoleSuffix();
						WPDB::query( "CREATE INDEX main_cache_key_index ON $cacheTableName(cache_key);" );
					}
				}
			}
		}
