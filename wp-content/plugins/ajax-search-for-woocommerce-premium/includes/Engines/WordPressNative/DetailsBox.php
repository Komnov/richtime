...
class DetailsBox {
		...
		if ( ! empty( $_POST['items'] ) && is_array( $_POST['items'] ) ) {
			...
			foreach ( $_POST['items'] as $item ) {
				...
				if ( dgoraAsfwFs()->is__premium_only() ) {
					...
					if ( ! empty( $postID ) && ! empty( $postType ) && $postType !== 'product' && $postType !== 'product_variation' ) {
						$items[] = array(
							'objectID' => $item['objectID'],
							'html'     => $this->getPostTypeDetails__premium_only( $postID )
						);
					}
				}
	...
	private function getProductDetails( $productID, $variationID = 0 ) {
		...
		if ( ( $product->isType( 'simple' ) || $product->isType( 'variation' ) )
		     && $wooProduct->is_purchasable()
		     && $wooProduct->is_in_stock()
		     && ! $wooProduct->is_sold_individually()
		     && apply_filters( 'dgwt/wcas/suggestion_details/show_quantity', true )
		) {
			$vars['showQuantity'] = true;
		}

		$vars = (object) apply_filters( 'dgwt/wcas/suggestion_details/product/vars', $vars, $productID, $product );
		...
		if ( $variationID ) {
			$details['html'] = apply_filters( 'dgwt/wcas/suggestion_details/product_variation/html', $details['html'], $variationID, $product );
		} else {
			$details['html'] = apply_filters( 'dgwt/wcas/suggestion_details/product/html', $details['html'], $productID, $product );
		}
	...
	/**
	 * Prepare details for post type
	 *
	 * @param int $postID
	 *
	 * @return string HTML
	 */
	private function getPostTypeDetails__premium_only( $postID ) {
		...
		$thumbSize        = apply_filters( "dgwt/wcas/suggestion_details/{$postType}/thumb_size", 'medium' );
		$responsiveImages = apply_filters( 'dgwt/wcas/suggestion_details/responsive_images', true );
		...
		$vars = array(
			...
			'imageSrc'    => $post->getThumbnailSrc( $thumbSize ),
			'imageSrcset' => $responsiveImages ? $post->getThumbnailSrcset( $thumbSize ) : '',
			'imageSizes'  => $responsiveImages ? $post->getThumbnailSizes( $thumbSize ) : '',
			'desc'        => $post->getDescription( 'details-panel' ),
			'postType'    => $postType
		);

		$vars = (object) apply_filters( 'dgwt/wcas/suggestion_details/' . $postType . '/vars', $vars, $postID, $post );

		ob_start();
		Helpers::loadTemplate( 'details-panel/' . $postType . '.php', $vars );
		$html = ob_get_clean();


		return apply_filters( 'dgwt/wcas/suggestion_details/' . $postType . '/html', $html, $postID, $postType );
	}
	...
	private function getTaxonomyDetails( $termID, $taxonomy, $termName ) {
		...
		if ( $products->have_posts() ) {
			...
			$title .= '</span>';
			$title = apply_filters( 'dgwt/wcas/suggestion_details/taxonomy/headline', $title, $termID, $taxonomy, $termName );
			...
			while ( $products->have_posts() ) {
				...
				if ( $product->isValid() ) {
					...
					$vars = (object) apply_filters( 'dgwt/wcas/suggestion_details/term_products/vars', $vars, $product->getID(), $product );
		...
		return apply_filters( 'dgwt/wcas/suggestion_details/term/html', $html, $termID, $taxonomy );
	}
