...
class Search {
	...
	public function __construct() {
		...
		if ( dgoraAsfwFs()->is__premium_only() ) {
			//  Run only if index isn't completed
			if ( DGWT_WCAS()->engine !== 'native' ) {
				return;
			}

			// Limit the number of fields to be searched using the native engine when the index is not ready
			add_filter( 'dgwt/wcas/native/search_in', function () {
				return array( 'title' );
			}, 10 );

			// Set 'placeholder' if the index is being built
			add_filter( 'dgwt/wcas/search_results/output', function ( $output ) {
				$status  = \DgoraWcas\Engines\TNTSearchMySQL\Indexer\Builder::getInfo( 'status', \DgoraWcas\Engines\TNTSearchMySQL\Config::getIndexRole() );
				$startTs = absint( \DgoraWcas\Engines\TNTSearchMySQL\Indexer\Builder::getInfo( 'start_ts', \DgoraWcas\Engines\TNTSearchMySQL\Config::getIndexRole() ) );
				if ( in_array( $status, array(
						'building',
						'preparing'
					) ) && ( time() - $startTs ) < 12 * HOUR_IN_SECONDS ) {
					$output['engine'] = 'placeholder';
				}
		...
		add_filter( 'posts_distinct', array( $this, 'searchDistinct' ), 501, 2 );
		...
		if ( Multilingual::isPolylang() && ! class_exists( 'Polylang_Woocommerce' ) && ! defined( 'Hyyan_WPI_DIR' ) ) {
			add_filter( 'woocommerce_ajax_get_endpoint', array( $this, 'fixPolylangWooEndpoint' ), 10, 2 );
		}
	...
	public function getSearchResults( $phrase = '', $return = false, $context = 'autocomplete' ) {
		...
		if ( apply_filters( 'dgwt/wcas/search_in_products', true ) ) {
			...
			do_action( 'dgwt/wcas/after_searching/products', $keyword, $totalProducts, $lang );
			...
			if ( ! empty( $products ) ) {
				...
				foreach ( $products as $post ) {
					...
					$orderedProducts[ $i ]->score = apply_filters('dgwt/wcas/search_results/product/score', $score, $keyword, $post->ID, $post );
			...
			if ( $i < $limit ) {
				...
				if ( $pos !== false ) {
					...
					$results[ $i ] = array(
						...
						'breadcrumbs' => Helpers::getTermBreadcrumbs( $cat->term_id, 'product_cat', array(), $termLang, array( $cat->term_id ) ),
						'type'        => 'taxonomy'
					);
