...
class WCMarketplace {
	...
	public static function getVendorData( $vendorID ) {
		...
		if ( is_numeric( $vendorID ) ) {
			$vendor = self::makeOfflineApiRequest( '\WCMp_REST_API_Vendors_Controller', 'get_item', array( 'id' => $vendorID ) );
		}
	...
	public function getVendors() {
		$vendors = array();
		$rawData = self::makeOfflineApiRequest( '\WCMp_REST_API_Vendors_Controller', 'get_items' );
	...
	public static function getVendorLogo( $imageID ) {
		$src   = '';
		$image = wp_get_attachment_image_src( $imageID, 'dgwt-wcas-product-suggestion' );
