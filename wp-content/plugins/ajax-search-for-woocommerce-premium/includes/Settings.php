...
class Settings {
	...
	public function __construct() {
		...
		if ( dgoraAsfwFs()->is__premium_only() ) {
			add_action( 'wp_ajax_dgwt_wcas_settings_list_custom_fields', array(
				$this,
				'listSearchableCustomFields__premium_only'
			) );
			add_action( 'wp_ajax_dgwt_wcas_settings_search_terms', array(
				$this,
				'searchTermsForFilter__premium_only'
			) );
		}
		...
		if ( dgoraAsfwFs()->is_premium() ) {
			...
			$sections[30] = array(
				...
				'title' => Helpers::getSettingsProLabel(
					...
					'header',
					__( 'with simple tricks', 'ajax-search-for-woocommerce' )
				)
			);
		}
	...
	function settingsFields() {
		...
		$settingsFields = array(
			...
			'dgwt_wcas_form_body'    => apply_filters( 'dgwt/wcas/settings/section=form', array(
				...
				645  => array(
					'name'    => 'search_style',
					'label'   => __( 'Style', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'search_style',
							sprintf( __( 'FiboSearch provides two different styles of search bars: Solaris and Pirx. Solaris has a rectangular shape while Pirx is a bean-shaped bar with curvy and smooth edges. <a target="_blank" href="%s">See the differences</a> between Solaris and Pirx style.',
								'ajax-search-for-woocommerce' ), $styleLink ) ),
					'type'    => 'select',
					'options' => array(
				...
				660 => array(
					'name'    => 'search_layout',
					'label'   => __( 'Layout', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'search_layout',
							__( 'FiboSearch might be displayed as a search bar, icon, or mixed of them.', 'ajax-search-for-woocommerce' )
							. ' <a target="_blank" href="' . $searchLayoutLink . '">' . __( 'See what each of these layouts looks like', 'ajax-search-for-woocommerce' ) . '</a>' ),
					'type'    => 'select',
					'options' => array(
				...
				680 => array(
					...
					'label'   => __( 'Overlay on mobile', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'enable_mobile_overlay',
							__( 'Covers anything that is displayed on the screen and simplifies the view to search bar + results.',
								'ajax-search-for-woocommerce' ) . ' ' . sprintf( $readMore, $mobileOverlayLink ) ),
				...
				695  => array(
					...
					'label'   => __( 'Darkened background', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'darken-_background',
							__( 'Darkening the page background while autocomplete is active gives it stronger emphasis, minimizing elements (e.g., ads, carousels, and other page content) that could distract users from considering autocomplete suggestions.',
								'ajax-search-for-woocommerce' ) . ' ' . sprintf( $readMore, $darkenedBgLink ) ),
...
//				),
				...
				2100 => array(
					...
					'label'   => __( 'Show Details panel', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'details-box',
							__( 'The Details panel is an additional container for extended information. The details change dynamically when the cursor hovers over one of the suggestions.', 'ajax-search-for-woocommerce' ) . ' ' . sprintf( $readMore, $detailsPanelLink ) ),
					'type'    => 'checkbox',
					'size'    => 'small',
...
//                ),
			...
			'dgwt_wcas_search'       => apply_filters( 'dgwt/wcas/settings/section=search', array(
				...
				150 => array(
					...
					'desc'    => dgoraAsfwFs()->is_premium() ? __( 'searching also in variable products SKU',
						'ajax-search-for-woocommerce' ) : sprintf( __( 'Searching in variable products SKU is available only in <a target="_blank" href="%s">the pro version</a>.',
				...
				520 => array(
					...
					'label' => __( 'Synonyms', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'synonyms',
							sprintf( __( 'The synonyms feature allows your users to find more relevant results. If your products have alternative names and users often misspell them, consider adding synonyms. <a target="_blank" href="%s">Read more</a> about this feature.', 'ajax-search-for-woocommerce' ),
								$synonymsLink ) ),
					'type'  => 'textarea',
					'desc'  => __( 'Synonyms should be separated by a comma. Each new synonyms group is entered in a new line. You can use a phrase instead of a single word. <br /> <br />Sample list:<br /> <br /><span class="dgwt-wcas-synonyms-sample">sofa, couch, davenport, divan, settee<br />big, grand, great, large, outsize</span>',
				...
				625 => array(
					...
					'label'   => __( 'Filtering mode', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'filter_products_mode',
							__( 'Exclude the product group from the search results or allow search only among the indicated product group.', 'ajax-search-for-woocommerce' ) . ' ' . sprintf( $readMore, $excludeIncludeLink ), '', 'right' ),
					'class'   => 'js-dgwt-wcas-adv-settings dgwt-wcas-premium-only',
					'type'    => 'select',
					'options' => array(
				...
				650 => array(
					...
					'label'   => __( 'Filters', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'filter_products_head',
							__( 'Filters that specify the product group and taxonomy that will be affected by the above mode', 'ajax-search-for-woocommerce' ), '', 'right' ),
			...
			'dgwt_wcas_performance'  => apply_filters( 'dgwt/wcas/settings/section=performance', array(
				...
				110 => array(
					...
					'label'   => __( 'Enable Scheduler', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'indexer-schedule',
							__( "In most cases, you don't need to use the scheduler because the search index updates when you edit products. If you use import tools or custom code to refresh prices or bulk add/edit products, the indexing scheduler will be helpful.",
								'ajax-search-for-woocommerce' ) . ' ' . sprintf( $readMore, $schedulerLink ) ),
					'type'    => 'checkbox',
					'size'    => 'small',
		...
		$fuzzinesText2 = sprintf( __( 'Returns suggestions based on likely relevance, even though a search keyword may not exactly match. E.g if you type “ipho<b>m</b>e” you get the same results as for “iphone”. <a target="_blank" href="%s">Read more</a> about the fuzzy search feature.', 'ajax-search-for-woocommerce' ),
			$fuzzySearchLink );
		...
		if ( dgoraAsfwFs()->is_premium() ) {
			...
			if ( dgoraAsfwFs()->is__premium_only() ) {
				...
				$settingsFields['dgwt_wcas_search'][300] = array(
					...
					'sanitize_callback' => array( '\DgoraWcas\Admin\SettingsAPI', 'sanitize_selectize_custom_fields__premium_only' ),
				...
				$settingsFields['dgwt_wcas_search'][450] = array(
					...
					'label'   => __( 'Fuzzy matching', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'fuzzy_search', $fuzzinesText2 ),
					'class'   => 'dgwt-wcas-premium-only',
					'type'    => 'select',
					'options' => array(
				...
				// Exclude/include products feature
				$settingsFields['dgwt_wcas_search'][600] = array(
					'name'  => 'filter_products_head',
					'label' => __( 'Exclude/include products', 'ajax-search-for-woocommerce' ),
					'type'  => 'head',
					'class' => 'dgwt-wcas-sgs-header js-dgwt-wcas-adv-settings'
				);
				$settingsFields['dgwt_wcas_search'][625] = array(
					'name'    => 'filter_products_mode',
					'label'   => __( 'Filtering mode', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'filter_products_mode',
							__( 'Exclude the product group from the search results or allow search only among the indicated product group.', 'ajax-search-for-woocommerce' ) . ' ' . sprintf( $readMore, $excludeIncludeLink ), '', 'right' ),
					'class'   => 'js-dgwt-wcas-adv-settings',
				...
				$settingsFields['dgwt_wcas_search'][650] = array(
					'name'              => 'filter_products_rules',
					'label'             => __( 'Filters', 'ajax-search-for-woocommerce' ) . ' ' . Helpers::createQuestionMark( 'filter_products_rules',
							__( 'Filters that specify the product group and taxonomy that will be affected by the above mode', 'ajax-search-for-woocommerce' ), '', 'right' ),
					'type'              => 'filters_rules__premium_only',
					'sanitize_callback' => array( '\DgoraWcas\Admin\SettingsAPI', 'sanitize_filters_rules__premium_only' ),
					'class'             => 'js-dgwt-wcas-adv-settings',
					'default'           => '[]',
				);
				...
				// Show next task description
				$settingsFields['dgwt_wcas_performance'][110]['desc'] = Helpers::isSettingsPage() ? Scheduler::nextTaskDescription() : '';

				// Remove promo box with list of Pro features
	...
	/**
	 * List searchable custom fields
	 * Ajax endpoint
	 *
	 * @return void
	 */
	public function listSearchableCustomFields__premium_only() {
		if ( ! current_user_can( 'administrator' ) ) {
			wp_die( - 1, 403 );
		}

		check_ajax_referer( 'dgwt_wcas_get_custom_fields' );

		$customFieldsTrans = get_transient( 'dgwt_wcas_searchable_custom_fields' );

		if ( ! empty( $customFieldsTrans ) && is_array( $customFieldsTrans ) ) {
			wp_send_json_success( $customFieldsTrans );
		}

		$list = Helpers::getSearchableCustomFields();

		set_transient( 'dgwt_wcas_searchable_custom_fields', $list, 60 );
	...
	/**
	 * List terms for filters
	 * Ajax endpoint
	 *
	 * @return void
	 */
	public function searchTermsForFilter__premium_only() {
		if ( ! current_user_can( 'administrator' ) ) {
			wp_die( - 1, 403 );
		}

		check_ajax_referer( 'dgwt_wcas_get_filter_terms' );

		$query = ! empty( $_POST['query'] ) ? wc_clean( wp_unslash( $_POST['query'] ) ) : '';
		$type  = ! empty( $_POST['type'] ) ? wc_clean( wp_unslash( $_POST['type'] ) ) : '';

		// Attribute taxonomy has form: "pa:X"
		if ( strpos( $type, 'pa:' ) !== false ) {
			$type = wc_attribute_taxonomy_name_by_id( (int) str_replace( 'pa:', '', $type ) );
		}
		...
		if ( Multilingual::isMultilingual() ) {
			$terms = Multilingual::getTermsInLang( $args );
		} else {
		...
		wp_send_json_success( array_values( $list ) );
	}
