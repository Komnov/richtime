...
function the_champ_counter_shortcode($params){
	...
	if(the_champ_social_counter_enabled()){
		...
		$html = '<div class="the_champ_counter_container the_champ_'.$type.'_counter' . ($type == 'vertical' && isset($theChampCounterOptions['hide_mobile_likeb']) ? ' the_champ_hide_sharing' : '') . '" ' . (the_champ_is_amp_page() ? '' : 'data-heateor-ss-offset="' . $alignmentOffset . '" ');
		$verticalOffsets = '';
		if($type == 'vertical'){
