...
class Richtime_Filter {
	...
	private function get_attributes_terms( $params ) {
		$terms     = [];
		
		foreach ( $params as $item ) {

			if( $item === 'brands' ) {
				$terms['brands']     = $this->get_brands();
				continue;
			}


			if( $item === 'individual' ) {
				$terms['individual'] = $this->get_individual_terms();
				continue;
			}
	...
	public function get_filters( array $names, array $params ) {
		$result = $this->get_attributes_terms( $params );
