<?php
/**
 * richtime functions and definitions
 *
...
if ( ! function_exists( 'richtime_setup' ) ) :
	/**
	 ...
	 */
	function richtime_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on richtime, use a find and replace
		 * to change 'richtime' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'richtime', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			[
				'menu-1' => esc_html__( 'Primary', 'richtime' ),
				'menu-2' => esc_html__( 'Footer', 'richtime' ),
				'menu-3' => esc_html__( 'Side', 'richtime' ),
			]
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'richtime_custom_background_args',
				[
...
add_action( 'after_setup_theme', 'richtime_setup' );
 ...
 */
function richtime_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'richtime_content_width', 640 );
}

add_action( 'after_setup_theme', 'richtime_content_width', 0 );
 ...
 */
function richtime_widgets_init() {
	register_sidebar(
		[
			'name'          => esc_html__( 'Сайдар для часов', 'richtime' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'richtime' ),
	...
	register_sidebar(
		[
			'name'          => esc_html__( 'Сайдаб для украшений', 'richtime' ),
			'id'            => 'sidebar-2',
			'description'   => esc_html__( 'Add widgets here.', 'richtime' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
...
add_action( 'widgets_init', 'richtime_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function richtime_scripts() {
	...
	wp_enqueue_style( 'richtime-style', get_stylesheet_uri(), [], _S_VERSION );
	wp_style_add_data( 'richtime-style', 'rtl', 'replace' );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}

	wp_enqueue_script( 'richtime-main', get_template_directory_uri() . '/build/index.js', [], _S_VERSION, true );
}

add_action( 'wp_enqueue_scripts', 'richtime_scripts' );

function richtime_language_switcher() {
	...
	if ( ! empty( $current_lang ) ) {
		echo sprintf( '<a class="richtime-language" href="%s">%s %s</a>', $current_lang['url'], $svg, $current_lang['tag'] );
	}
 ...
 * Richtime widgets
 */
require get_template_directory() . '/inc/widgets/class-richtime-attribute-filter.php';
 ...
 * Filters
 */
require get_template_directory() . '/inc/filters/class-richtime-filter.php';
